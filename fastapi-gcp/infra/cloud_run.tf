...
# Cloud Run のランタイム用サービスアカウント（デフォルトの Compute SA は権限が広いため使わない）
...
resource "google_cloud_run_v2_service" "app" {
  ...
  # 検証用途のため destroy 可能にしておく
  deletion_protection = false

  # IAP ダイレクト統合ではリクエストが IAP を経由して Cloud Run に届くため
  # ingress は全許可のままにし、認証は IAP + run.invoker で制御する
  ingress = "INGRESS_TRAFFIC_ALL"

  # Cloud Run の IAP ダイレクト統合を有効化
  iap_enabled = true
