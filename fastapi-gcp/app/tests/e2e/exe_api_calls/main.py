"""Call the deployed POST endpoints through IAP and write the responses to a file.
...
case, e.g. data/echo/hello.json is POSTed to /echo and recorded under the case
name "echo/hello". Adding a case means adding a file there, with no change to
this script.
...
OUTPUT_PATH = Path(__file__).parents[1] / "results.json"
...
def main() -> None:
    """POST every test data file to its endpoint and write the responses to one file.

    Input: data/<endpoint>/<case>.json, each holding a request body. The parent
    directory name is the endpoint (data/echo/hello.json is POSTed to /echo) and
    the two combine into the case name "echo/hello".

    Output: ../results.json, i.e. tests/e2e/results.json, holding every case
    keyed by that name alongside the service URL and the collection timestamp.
    tests/e2e/test_api.py reads that file and verifies it.
    """
    token = sign_jwt()
    ...
    for testdata_path in testdata_paths:
        path = f"/{testdata_path.parent.name}"
        name = f"{testdata_path.parent.name}/{testdata_path.stem}"
        cases[name] = call(path, json.loads(testdata_path.read_text()), token)
        print(f"{name}: POST {path} -> {cases[name]['status_code']}")
